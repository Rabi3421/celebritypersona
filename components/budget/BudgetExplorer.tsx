...
import { isFullySwapped, pricing, wornPrice, wornSortKey } from "@/lib/types";
...
export function BudgetExplorer({ outfits, initialBudget }: { outfits: Outfit[]; initialBudget?: number }){
 ...
 const eligible=useMemo(()=>outfits.filter(isFullySwapped).filter((outfit)=>pricing(outfit).swapTotal<=budget).sort((a,b)=>wornSortKey(b)-wornSortKey(a)),[outfits,budget]);
 ...
 const biggestGap=eligible[0]?wornPrice(eligible[0]):null;const progress=(budget-range.min)/Math.max(1,range.max-range.min)*100;
 const cheapestOverall=useMemo(()=>{const totals=outfits.filter(isFullySwapped).map((outfit)=>pricing(outfit).swapTotal);return totals.length?Math.min(...totals):null;},[outfits]);
 return <main className={styles.page}>
  <header className={styles.dial}><div className={styles.shell}><nav><Link href="/">Home</Link><i>›</i><span>Budget</span></nav><h1>Celebrity-inspired outfits<br/>by what you can spend.</h1><p>Everyone else starts with the celebrity. Start with your wallet: set what you&apos;re willing to spend on a complete look and we&apos;ll show you exactly which ones you can copy, piece by piece.</p><div className={styles.knob}><div><p className={styles.amount}><span>₹</span>{budget.toLocaleString("en-IN")}</p><div className={styles.slider}><input type="range" min={range.min} max={range.max} step={range.step} value={budget} style={{"--progress":`${progress}%`} as CSSProperties} onChange={(event)=>setBudget(Number(event.target.value))} aria-label="Your budget"/><div><span>{inr.format(range.min)}</span><span>{inr.format(range.max)}</span></div></div><div className={styles.presets}>{range.presets.map((value)=><button type="button" aria-pressed={budget===value} onClick={()=>setBudget(value)} key={value}>{value===range.max?"No limit":inr.format(value)}</button>)}</div></div><div className={styles.readout}><p><span>Complete looks you can copy</span><b>{eligible.length}</b></p><p><span>Occasions covered</span><b>{coverage.filter((entry)=>entry.looks).length} of {occasionList.length}</b></p>{/* Both of these printed an em dash with their label still attached, so at a
    budget nothing fits the panel read "Cheapest complete look —" and "You can
    copy a look that originally cost —". A dash is a mark standing in for data
    and the label turns it into a promise of data we do not have; when there is
    no figure, the line goes. */}
{eligible.length?<p><span>Cheapest complete look</span><b>{inr.format(Math.min(...eligible.map((outfit)=>pricing(outfit).swapTotal)))}</b></p>:null}{biggestGap?<div><span>◆ The biggest gap at this budget</span><p>You can copy a look that originally cost<b>{compactPrice(biggestGap)}</b></p></div>:null}</div></div></div></header>
...
function BudgetCard({outfit,budget,saved,onSave}:{outfit:Outfit;budget:number;saved:boolean;onSave:()=>void}){const money=pricing(outfit);return <article className={styles.card}><Link href={`/outfits/${outfitSlug(outfit)}`}><div><OutfitThumb outfit={outfit} sizes="(max-width:520px) 50vw, 25vw"/><span>{outfit.occasion}</span><b>fits {inr.format(budget)}</b></div><section><h3>{outfit.celebrity}</h3><p>{outfit.event}</p><span>{money.anyPriced?<s>{inr.format(money.wornTotal)}</s>:null}<b>{inr.format(money.swapTotal)}</b></span></section></Link><button type="button" aria-pressed={saved} onClick={onSave}>{saved?"♥":"♡"}</button></article>;}
