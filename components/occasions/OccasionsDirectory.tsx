...
/**
 * What the date beside an upcoming occasion means.
 *
 * A festival has one date: Diwali is on the 8th. A wedding occasion does not —
 * Sangeet's stored date is the start of the season, because sangeets happen
 * all through it. Printing "Sangeet · 15 Nov" told readers there is a sangeet
 * on the 15th, which is not something this site knows or could know.
 */
const dateLabel = (event: OccasionView) => {
  const day = formatDay(event.nextDate);
  if (!day) return "";
  return event.group === "Wedding" ? `Season starts ${day}` : day;
};
...
export async function OccasionsDirectory() {
  ...
  return <main className={styles.page}>
    ...
    {upcoming.length ? <section className={styles.calendar}><div className={styles.shell}><div className={styles.calendarHeading}><span>◆ Coming up</span><i/><small>Dates approximate</small></div><div className={styles.calendarRail}>{upcoming.map((event)=><Link href={`/occasions/${occasionSlug(event)}`} className={(event.daysAway ?? 0)<=SOON_DAYS?styles.soon:""} key={event.id}><div><span><h2>{event.name}</h2><small>{dateLabel(event)}</small></span><i style={{"--progress":`${Math.max(8,100-(event.daysAway ?? 0)/1.2)}%`} as React.CSSProperties}><b>{event.daysAway}</b><small>days</small></i></div><p><span>Looks ready</span><b>{event.stats.looks}</b></p></Link>)}</div></div></section> : null}
