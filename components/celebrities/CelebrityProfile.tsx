...
export function CelebrityProfile({ celebrity, outfits, similar }: { celebrity: CelebrityView; outfits: Outfit[]; similar: CelebrityView[] }) {
  ...
  const affordable = priced.length ? [...priced].sort((a,b) => wornSortKey(a) - wornSortKey(b))[0] : null;
  const expensive = priced.length ? [...priced].sort((a,b) => wornSortKey(b) - wornSortKey(a))[0] : null;
  // Real counts: how often each label actually appears across her pieces.
  // What this section is allowed to claim depends on how much of her is in
  // the archive. One decoded look cannot show a "palette she returns to", and
  // saying so about a real person on the strength of a single outfit is the
  // kind of confident wrongness a reader spots immediately.
  const pattern = patternLanguage(stats.looks);
  ...
  return (
    <main className={styles.page}>
        ...
        <div className={styles.shell}>
          ...
          <div className={styles.heroInner}>
            <figure className={styles.portrait}>
              <CelebrityThumb celebrity={celebrity} preload sizes="(max-width:1023px) 220px, 340px" />
            ...
            <div className={styles.heroCopy}>
              ...
              <p className={styles.byline}>Written by <Link href="/authors/rabi"><b>Rabi</b></Link>{checked ? <> · Prices re-checked <b><time dateTime={stats.lastChecked ?? undefined}>{checked}</time></b></> : null} · <Link href={`/report-a-price?piece=${encodeURIComponent(celebrity.name)}`}>Report a correction</Link></p>
              <div className={styles.heroStats}>
                ...
                {stats.low === null || stats.high === null || stats.low === stats.high ? null : <div><b>{compactPrice(stats.low)}–{compactPrice(stats.high)}</b><span>Typical range</span></div>}
              </div>
        ...
        <div className={styles.shell}>
          ...
          <div className={styles.signatureGrid}>
            ...
            <div className={styles.patternPanel}>
              ...
              {stats.low === null || stats.high === null ? null : stats.low === stats.high ? <div><h3>What this look cost</h3><p className={styles.tags}><span>{inr.format(stats.low)} as worn</span></p></div> : <div><h3>What her looks cost</h3><div className={styles.range}><span>Cheapest</span><span>Priciest</span><i><b /></i><strong>{inr.format(stats.low)}</strong><strong>{inr.format(stats.high)}</strong></div></div>}
            </div>
          </div>
        </div>
      </section>
      ...
      {similar.length ? <section className={styles.similar}><div className={styles.shell}><SectionHeading eyebrow="Nearby" title="Similar style archives" /><div className={styles.similarGrid}>{similar.map((item) => <Link href={`/celebrities/${celebritySlug(item)}`} key={item.id}><span className={styles.similarThumb}><CelebrityThumb celebrity={item} decorative sizes="78px" /></span><b>{item.name}</b><span>{plural(item.stats.looks, "look")}</span></Link>)}</div></div></section> : null}
    </main>
  );
}

/** Four looks is where recurrence starts meaning something. Below it, the
 *  headings describe the archive rather than the person. */
function patternLanguage(looks: number) {
  if (looks >= 4) {
    return {
      eyebrow: "The pattern",
      title: "What she actually wears",
      body: `Counted across all ${looks} decoded looks — not an impression, the actual tally.`,
      labelsHeading: "Most repeated labels",
      paletteHeading: "Palette she returns to",
      garmentHeading: "Silhouettes she goes to",
    };
  }
  return {
    eyebrow: "In the archive",
    title: looks === 1 ? "What this look contains" : `What these ${looks} looks contain`,
    body:
      looks === 1
        ? "One decoded look. Too few to describe how she dresses, so this is simply what is in it."
        : `Counted across the ${looks} looks decoded so far — an observation from the archive, not yet a pattern.`,
    labelsHeading: "Labels worn",
    paletteHeading: looks === 1 ? "Colours in this look" : "Colours in these looks",
    garmentHeading: looks === 1 ? "Pieces decoded" : "Garments decoded",
  };
}
